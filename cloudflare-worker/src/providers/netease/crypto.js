// Web Crypto API adapter for Cloudflare Workers
// Replaces crypto-browserify + buffer
...
async function aesEncrypt(plaintext, mode, key, iv) {
  const algoName = mode === 'ecb' ? 'AES-ECB' : 'AES-CBC'
  const algo = { name: algoName }
  if (mode !== 'ecb') algo.iv = iv

  const cryptoKey = await crypto.subtle.importKey('raw', key, { name: algoName }, false, ['encrypt'])
  const encrypted = await crypto.subtle.encrypt(algo, cryptoKey, plaintext)
...
const eapi = async (url, object) => {
  ...
  const digestBuffer = await crypto.subtle.digest('MD5', textToBytes(message)).catch(() => {
    // MD5 might not be available, use a simple hash fallback
    // Actually Cloudflare Workers support MD5 via subtle crypto
    return null
  })

  // Cloudflare Workers don't support MD5 in subtle crypto
  // Use a simple MD5 implementation
  const digest = await md5(textToBytes(message))
...
export default { weapi, linuxapi, eapi }
