...
const { net_ease_anonymous_token } = require('./config.js');
...
// nanoid replacement - simple random hex string
function nanoid() {
...
const request = async (method, url, data = {}, options) => {
    ...
    if (options.crypto === 'weapi') {
        ...
        data = await encrypt.weapi(data)
        url = url.replace(/\w*api/, 'weapi')
    } else if (options.crypto === 'linuxapi') {
        data = await encrypt.linuxapi({
            method: method,
        ...
        if (cookie.osver) header['osver'] = cookie.osver
        if (cookie.deviceId) header['deviceId'] = cookie.deviceId
        if (cookie.mobilename) header['mobilename'] = cookie.mobilename
        if (cookie.channel) header['channel'] = cookie.channel
        if (cookie.MUSIC_U) header['MUSIC_U'] = cookie.MUSIC_U
        if (cookie.MUSIC_A) header['MUSIC_A'] = cookie.MUSIC_A

        // Build cookie string, filtering out undefined values
        headers['Cookie'] = Object.keys(header)
            .filter(key => header[key] !== undefined && header[key] !== null && header[key] !== '')
            .map((key) => encodeURIComponent(key) + '=' + encodeURIComponent(header[key]))
            .join('; ')
        data.header = header
        data = await encrypt.eapi(options.url, data)
...
module.exports = { request, map_song_list };
